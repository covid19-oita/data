...
def main():
    csvfile_dir = "/../../csv/"
    patients_csvfile = os.path.dirname(__file__) + csvfile_dir + PATIENTS_CSVFILE
    data_summary_csvfile = (
        os.path.dirname(__file__) + csvfile_dir + DATA_SUMMARY_CSVFILE
    )
    ...
    fdh = fdhandler.FinancialDataHandler(
        ...
        loan_amount_data_csv=os.path.dirname(__file__)
        + csvfile_dir
        + "440001oitacovid19finamount.csv",
        loan_counts_data_csv=os.path.dirname(__file__)
        + csvfile_dir
        + "440001oitacovid19finnumber.csv",
        industry_master_data_csv=os.path.dirname(__file__)
        + csvfile_dir
        + "440001oitacovid19fintype.csv",
        subsidy_data_csv=os.path.dirname(__file__)
        + csvfile_dir
        + "440001oitaemploymentsubsidy.csv",
    )
